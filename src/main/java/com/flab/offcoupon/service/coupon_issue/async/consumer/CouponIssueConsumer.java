 ...
 * <ol>
 *     <li>메시지 큐에 메시지가 있는 경우, 메시지를 받아와서 쿠폰 발행 이력을 저장합니다.</li>
  ...
  //  @Scheduled(fixedDelay = 3000)
    public void consumeCouponIssueMessage() throws IOException {
        if (existCouponIssueQueueTarget()) {
            // 큐에서 메세지 consume
            message = (CouponIssueMessageForQueue) rabbitTemplate.receiveAndConvert("coupon-issue.queue");
    ...
    private boolean existCouponIssueQueueTarget() throws IOException {
        return messageQueueCountChecker.getMessageCount("coupon-issue.queue") > 0;
    }
    /**
     * 각 쿠폰 발행 이력을 저장합니다.
    ...
    /**
     * MySQL에 저장된 총 발행된 수량을 업데이트하고, Redis에 저장된 요청 중 메시지로 들어온 쿠폰ID와 관련된 데이터를 삭제합니다.
     */
