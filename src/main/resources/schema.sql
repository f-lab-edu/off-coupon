...
(
    ...
    status              VARCHAR(50)     NOT NULL DEFAULT 'NOT_ACTIVE' COMMENT '유효일 전 : NOT_ACTIVE / 유효기간 : ACTIVE / 사용완료 : USED / 만료 : EXPIRED',
...
(
    coupon_issue_id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY COMMENT '쿠폰 발행 기록',
    member_id       BIGINT UNSIGNED NOT NULL COMMENT '쿠폰 ID',
    coupon_id       BIGINT UNSIGNED NOT NULL COMMENT '회원 ID',
    isUsed          VARCHAR(255)    NOT NULL COMMENT '사용 여부',
    created_at      DATETIME        NOT NULL COMMENT '데이터 생성일',
    updated_at      DATETIME        NOT NULL COMMENT '데이터 변경일'
);
