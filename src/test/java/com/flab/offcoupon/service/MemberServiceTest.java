...
class MemberServiceTest {
	...
	void singup_success() {
		// given
		SignupMemberRequestDto validSignupMemberRequestDto = SignupMemberRequestDto.create("seijin0722@naver.com",
			"abcabc123", "name", LocalDate.parse("2024-12-12"), "010-1234-1234", Role.ROLE_USER);
		//doNothing().when(memberRepository).save(any());
		// when(memberRepository.save(any(Member.class)));
		// given(memberRepository.save(any(Member.class)));
		ResponseDTO responseDTO = sut.signUp(validSignupMemberRequestDto);
	...
	void singup_fail_duplicated_email() {
		// given
		SignupMemberRequestDto duplicateSignupMemberRequestDto = SignupMemberRequestDto.create("seijin0722@naver.com",
			"abcabc123", "name", LocalDate.parse("2024-12-12"), "010-1234-1234", Role.ROLE_USER);

		// when 이미 해당 이메일이 존재한다고 가정
		when(memberRepository.existMemberByEmail(any())).thenReturn(true);
		// then
		assertThatThrownBy(() -> sut.signUp(duplicateSignupMemberRequestDto))
