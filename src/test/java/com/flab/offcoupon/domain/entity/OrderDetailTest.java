...
class OrderDetailTest {
	...
	class createOrderDetail {
		...
		void createOrderDetail_success(long productId, long quantity, BigDecimal pricePerEach,
			List<AppliedCouponInfo> appliedCouponInfos) {
			// Given
		...
		private static Stream<Arguments> validParams() {
			...
			Coupon coupon = new Coupon(1L, 1L, DiscountType.PERCENT, 20L, null, CouponType.FIRST_COME_FIRST_SERVED,
				500L, 10L, LocalDateTime.now(), LocalDateTime.now(), LocalDateTime.now(), LocalDateTime.now());
			Product product = new Product(1L, "category", "title", "description", BigDecimal.valueOf(1000),
				BigDecimal.valueOf(0), BigDecimal.valueOf(1000), null, null);
		...
		void createOrderDetail_fail_with_invalid_Id_and_quantity(long productId, long quantity) {
			// Given
			OrderInfo orderInfo = new OrderInfo(productId, quantity, BigDecimal.valueOf(20000L),
