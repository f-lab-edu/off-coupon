...
class CouponIssueControllerTest {
	...
	void issue_coupon_fail_invalid_event_period() throws Exception {
		...
		ResponseDTO failResponse = ResponseDTO.getFailResult(
			INVALID_EVENT_PERIOD.formatted("2021-01-01", "2021-01-01"));
		given(couponIssueRequestService.syncIssueCoupon(any(LocalDateTime.class), any(IssueRequestParameter.class)))
	...
	void sync_issue_coupon_success() throws Exception {
		...
		info.add("couponId", String.valueOf(couponId));
		info.add("memberId", String.valueOf(memberId));
		ResponseDTO successResult = ResponseDTO.getSuccessResult(
