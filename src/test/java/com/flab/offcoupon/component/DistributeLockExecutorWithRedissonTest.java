...
import static com.flab.offcoupon.util.LockMagicNumberConstants.*;
...
class
DistributeLockExecutorWithRedissonTest {
	...
	void execute_catch_block_exception() throws InterruptedException {
		...
		when(redissonClient.getLock(lockName)).thenReturn(lock);
		when(lock.tryLock(LOCK_WAIT_MILLI_SECOND, LOCK_LEASE_MILLI_SECOND, TimeUnit.MILLISECONDS)).thenThrow(
